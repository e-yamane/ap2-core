...
COMMENT ON COLUMN ENTRY.QUANTITY_UNIT_ID IS '移動量 数量尺度ID';
COMMENT ON COLUMN ENTRY.QUANTITY_A_VALUE IS '移動量 量 量(整数)';
COMMENT ON COLUMN ENTRY.QUANTITY_A_SCALE IS '移動量 量 小数点位置。正の数なら左へ、負の数なら右へ移動させる';
